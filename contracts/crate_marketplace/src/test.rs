...
mod tests {
    ...
    #[test]
    fn test_purchase_only_moves_the_pinned_payment_token() {
        let env = Env::default();
        env.mock_all_auths();
        let platform = Address::generate(&env);
        let buyer = Address::generate(&env);

        // Buyer holds funds in two independent token contracts. The contract
        // is pinned to `accepted_token` at construction; `decoy_token` stands
        // in for an attacker-deployed no-op token. Since purchase_license no
        // longer accepts a caller-supplied token address (see issue #8), there
        // is no parameter through which the buyer could redirect payment to
        // the decoy — this test confirms the decoy is never touched.
        let (accepted_token, accepted_client) = create_xlm_token(&env, &buyer);
        let (_decoy_token, decoy_client) = create_xlm_token(&env, &buyer);

        let contract_id = env.register(CrateMarketplace, (&1000u32, &platform, &accepted_token));
        let client = CrateMarketplaceClient::new(&env, &contract_id);
        assert_eq!(client.get_payment_token(), accepted_token);

        let producer = Address::generate(&env);
        let sample_id = client.upload_sample(
            &producer,
            &String::from_str(&env, "Pinned Token Test"),
            &String::from_str(&env, "QmPinnedCID"),
            &100_000_000i128,
            &500_000_000i128,
            &2_000_000_000i128,
            &String::from_str(&env, "Trap"),
        ...
        // Payment was settled in the accepted token...
        assert_eq!(accepted_client.balance(&buyer), 9_900_000_000i128);
        assert_eq!(accepted_client.balance(&platform), 10_000_000i128);
        // ...and the decoy token, which a malicious buyer would prefer to pay
        // with, was never invoked at all.
        assert_eq!(decoy_client.balance(&buyer), 10_000_000_000i128);
        assert_eq!(decoy_client.balance(&platform), 0i128);
    }
